...
import { embedPassage, embedQuery } from "./embedding-nvidia.js";
...
const searchMilvus = async (
  ...
  const buildSearchBody = (embedding: number[]) => ({
...
export const retrieveContext = async (query: string): Promise<string> => {
  try {
    ...
    const [queryVector, passageVector] = await Promise.all([
      embedQuery(`query: ${query}`),
      embedPassage(`passage: ${query}`)
    ]);
